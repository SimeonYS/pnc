...
class PncPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('pnc.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pnc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pnc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pnc (date, title, link, content)"
